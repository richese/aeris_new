...
int ae::Server::main()
{
  ...
  while (!signal::exit())
  {
    ...
    if (!request_handlers.empty())
    {
      ...
      {
        if (it->wait_for(time::milliseconds(0)) == std::future_status::ready)
        {
...
void ae::Server::update_global_state(const std::vector<sAgentInterface> &agents)
{
  ...
  // extract agent group id
  uint64_t group = agents[0].id & (0xFFFFFFFF00000000);
  // find position of first agent from this group in global state map
  auto group_it = m_global_state.lower_bound(group);

  if (group_it == m_global_state.end())
  {
    // first commit of newly registered agent group just inserts to the end
