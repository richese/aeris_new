 ...
 */
Socket::Socket(const char *ip, const int portno) :
  m_fd(-1),
  m_family(AF_INET),
  m_portno(portno),
  m_address(),
...
Socket::Socket(int fd, int family, const char *address, int portno) :
  m_fd(fd),
  m_family(family),
  m_portno(portno),
  m_type(TYPE_ACCEPT)
...
void Socket::log(el::base::type::ostream_t& os) const
{
  os << "Socket(type=";
  switch (m_type)
  {
...
void SocketWatch::add_socket(std::shared_ptr<Socket> sock)
{
